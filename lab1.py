...
def create_y(a: float, b: float, c: float) -> float:
    ...
    Функция принимает три параметра a, b и c и возвращает анонимную функцию,
    которая вычисляет значение y по формуле:
...
def load_params(filename: str) -> dict:
    ...
    Файл должен содержать пары ключ-значение в формате "ключ": значение, разделенные
    запятыми или пробелами. Функция читает файл, извлекает пары ключ-значение с 
    помощью регулярного выражения и возвращает словарь, содержащий параметры.
...
def calc_in_range(func: Callable, lower: float, upper: float, step: float) -> list:
    ...
    Функция принимает вызываемую функцию, нижнюю границу, верхнюю границу и
    размер шага. Она вычисляет значения функции для каждого значения в диапазоне
    от нижней границы до верхней границы (включительно) с заданным размером шага.
